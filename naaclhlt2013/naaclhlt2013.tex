...
\title{Question and Answering System}
...
\maketitle
\begin{abstract}
  Every question has an expected answer type. We present a method for constructing a probability-based answer type model for each different question. Our model evalutes the probability that a candidate answer fits into the given question semantical context. Evaluation is performed on a set of question sets created manually and from NTCR.
\end{abstract}
...
Natural language data is extremely sparse. Word cluster is a way of solving data sparseness by abstracting a given word to a class of related words.

We used the Brown hierarchical word clustering algorithm implemented by Percy Liang on Chinese wikipedia to obtain 1000 clusters. 
\cite{liang2005}
\cite{pinchak2006}
\cite{brown1992class}

The cluster to which a word belongs often represent the senses of the word.
